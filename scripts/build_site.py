...
REPOSITORY_URL: Final[str] = "https://github.com/kogunlowo123/ai-model-gateway"
...
PAGES: Final[tuple[Page, ...]] = (
    ...
    Page(Path("docs/routing.md"), "routing", "Routing and the estimator", "Reference"),
    Page(Path("docs/resilience.md"), "resilience", "Retries, breakers, capacity", "Reference"),
    Page(Path("docs/simulator.md"), "simulator", "The simulator, and its limits", "Reference"),
    Page(Path("docs/upstreams.md"), "upstreams", "Upstreams and cost", "Reference"),
    Page(Path("docs/http.md"), "http", "The HTTP surface", "Reference"),
...
#: Repository-relative link targets that do not exist. Collected during
#: rendering and raised by `verify`, so one build reports every broken link
#: rather than only the first.
_MISSING: Final[list[str]] = []
...
def _rewrite_links(rendered: str, *, source: Path) -> str:
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        #
        # But only if it is actually there. Without this check a typo'd path
        # turns into a well-formed GitHub URL to a file that does not exist,
        # which `verify` below cannot see because it only looks at links that
        # stayed relative. That is the exact shape of gate this project exists
        # to argue against: one that reports success on input it never checked.
        target_path = REPO_ROOT / normalised
        if not target_path.exists():
            _MISSING.append(f"{source.as_posix()} -> {normalised}")
...
def verify(output: Path) -> None:
    ...
    if _MISSING:
        broken.extend(f"missing in the repository: {entry}" for entry in _MISSING)
    if broken:
...
def build(output: Path) -> int:
    ...
    for page in PAGES:
        ...
        (output / page.output_name).write_text(
            TEMPLATE.format(
                title=html.escape("amg" if page.slug == "index" else f"{page.title} — amg"),
